...
func (c *Client) GenerateNarration(ctx context.Context, text, audioType, inputType string) (string, error) {
	...
	case "podcast":
		audioStyle = "Professional podcast style: write as a discussion between two distinct voices. Use exactly these speaker labels before each line: '" + PodcastSpeaker1 + ":' and '" + PodcastSpeaker2 + ":' so it reads as a natural two-person conversation, with good pacing and emphasis."
	default:
